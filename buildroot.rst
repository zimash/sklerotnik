...
Buildroot - это в основном набор Makefile-ов, что скачивает, конфигурирует, компилирует ПО с разными опциями + набором патчей для некоторого ПО.

По сути, существует один Makefile для каждого пакета программного обеспечения с расширением .mk. Makefile-ы разделены на множество различных частей::

    toolchain/ # Makefile-ы и связанные файлы для ПО, связанного с кросскомпилляционными тулчейнами: binutils, gcc, gdb, kernel-headers и uClibc.
    arch/      # директория содержит объявления для всех процессорных архитектур, что поддерживаются Buildroot-ом.
    package/   # Makefile-ы и связанные файлы для user-space инструментов и библиотек, что в Buildroot может компилироваться и добавляться в target root ФС. Одна поддиректория на пакет.
    linux/     # Makefile-ы и связанные файлы для ядра Linux.
    boot/      # Makefile-ы и связанные файлы для загрузчиков, поддерживаемых Buildroot-ом.
    system/    # содержит поддержку системной интеграции, например, скелет target ФС и выбор системы инициализации.
    fs/        # Makefile-ы и связанные файлы для ПО связанное с генерацией образа целевой корневой файловой системы.
